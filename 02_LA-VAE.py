...
class CustomCallback(Callback):  
    ...
    def save_new_sequences_and_plot(self, epoch):  
        ...
        with open(f'VAE_TastePeps_{self.dataset_name}_Epoch_{epoch+1}.txt', 'w') as file:  
            for idx, seq in enumerate(sequences_decoded):  
                file.write(f"Index: {idx}, Sequence: {seq}\n")  

        z_mean, _ = encoder.predict(self.sequences)  
        z_mean_generated, _ = encoder.predict(x_decoded)  
        ...
        print(f"New sequences for epoch {epoch+1} have been saved.")  
                ...
                for line in f:  
                    if line.strip():  
                        ...
                        if idx in selected_indices:  
                            seq = line.split(',')[1].split(':')[1].strip()  
                            sequences.append({  
                                'index': idx,  
